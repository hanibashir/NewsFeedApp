...
class simple_html_dom_node {
    ...
    function text()
    {
        ...
        if (strcasecmp($this->tag, 'css')===0) return '';
...
class simple_html_dom {
    ...
    function load($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT) {
        ...
        // Script tags removal now preceeds css tag removal.
        // strip out <script> tags
        $this->remove_noise("'<\s*script[^>]*[^/]>(.*?)<\s*/\s*script\s*>'is");
        $this->remove_noise("'<\s*script\s*>(.*?)<\s*/\s*script\s*>'is");
        // strip out <css> tags
        $this->remove_noise("'<\s*css[^>]*[^/]>(.*?)<\s*/\s*css\s*>'is");
        $this->remove_noise("'<\s*css\s*>(.*?)<\s*/\s*css\s*>'is");
