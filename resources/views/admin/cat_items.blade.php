        ...
        <table class="table table-responsive table-hover">
            ...
            @foreach($cat_items as $item)
                <tr>
                    ...
                    @endif
                    <td><a href="{{ url('/') }}/items/{{$item->id}}" target="_blank">{{html_entity_decode(strip_tags($item->title)) }}</a></td>

                    <td>
                        @if(isset($item->feed->id))
                            <a href="{{ url('/admin') }}/feeds/{{$item->feed->id}}/items"> {{$item->feed->feed_name}}</a>
                        @else
                        {{Auth::user()->name }}
                        @endif
                        </td>
